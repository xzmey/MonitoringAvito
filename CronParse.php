...
const ACCESS_KEY = "c6ec3d30";  // ключ из сообщества
...
foreach ($urlsAll as $url=>$value)
{
    ...
    foreach ($url as $key=>$ad)
    {
        ...
        if (!(R::findOne('ads','url_ad=?',array( $urlAd))))
        {
            ...
            ✅ Посмотреть остальные объявления тут -  http://c9fd9fb111d8.ngrok.io
            Держим вас в курсе🤙🏻
            ");
            sleep(rand(1,4));
            }
            if ($newCount==10)
            {
                $vk->sendMessage($id['user_id'], "У вас больше 10 новых объявлений, загляните на станицу -  http://c9fd9fb111d8.ngrok.io     
            ");
            }
        }
